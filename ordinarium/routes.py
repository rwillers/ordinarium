...
def get_user(email='ryanwillers@gmail.com'):
	db = get_db()
	user = db.execute('select id, first_name, last_name, email from users where email=? limit 1', (email,)).fetchone()
	return user
...
def plan(rite='Renewed Ancient Text'):
	...
	saved_plan = db.execute('select text_order, text_disabled from services where id=? limit 1', (1,)).fetchone()
	if saved_plan:
		ordinaries = db.execute('select texts.id, texts.default_order, texts.title, texts.detailed_title, texts.text, saved_order.value, saved_disabled.value as "disabled" from texts left join json_each(?) saved_order on texts.id=saved_order.value left join json_each(?) saved_disabled on texts.id=saved_disabled.value where texts.type=? and texts.filter_type=? and texts.filter_content=? order by saved_order.key', (saved_plan['text_order'], saved_plan['text_disabled'], 'ordinarium', 'rite', rite)).fetchall()
	else:
	...
	return render_template('plan.html', user=get_user(), rite=rite, rite_slug=rite_slug, ordinaries=ordinaries)

@bp.route('/text/<rite_slug>')
def text(rite_slug):
	if not rite_slug:
		return render_template('page.html', title='Error', content='No rite indicated'), 404
	...
	rite = db.execute('select distinct filter_content from texts where type=? and filter_type=? and filter_content=? limit 1', ('ordinarium', 'rite', rite_name)).fetchone()
	if not rite:
		return render_template('page.html', title='Error', content='Rite not found'), 404
	
	ids = request.args.get('ids', '').split(',') if request.args.get('ids') else []
	if ids:
		ids_json = '[' + ','.join(ids) + ']'
		ordinaries = db.execute('select title, text from texts join json_each(?) ids on texts.id=ids.value where texts.type=? and texts.filter_type=? and texts.filter_content=? order by ids.key;', (ids_json, 'ordinarium', 'rite', rite_name)).fetchall()
	else:
		ordinaries = db.execute('select title, text from texts where type=? and filter_type=? and filter_content=? order by default_order;', ('ordinarium', 'rite', rite_name)).fetchall()
	if not ordinaries:
		return render_template('page.html', title='Error', content='Content not found'), 404
	...
	return render_template('text.html', title=title, rite=rite_name, ordinaries=ordinaries, **propers)

@bp.route('/persist/service', methods=['POST'])
def persist_service():
	order = request.form.get('ids', '').split(',') if request.form.get('ids') else []
	order_json = '[' + ','.join(order) + ']'
	disabled = request.form.get('disabled', '').split(',') if request.form.get('disabled') else []
...
def page(slug):
	db = get_db()
	page = db.execute('select title, content from pages where slug=? limit 1', (slug,)).fetchone()
	if page:
